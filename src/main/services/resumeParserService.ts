...
export class ResumeParserService {
  static parseResumeContent(content: string): ParsedResumeInfo {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!result.phone) {
        const phoneMatch = line.match(/1[3-9]\d{9}|(\+86)?1[3-9]\d{9}|\d{3,4}-\d{7,8}/);
  ...
  private static parseEducationLine(line: string, currentEducation: any): void {
    ...
    if (!currentEducation.degree && /学士|硕士|博士|本科|专科|Bachelor|Master|PhD/i.test(line)) {
    ...
    if (!currentEducation.period && /\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{4}\.\d{1,2}\.\d{1,2}/.test(line)) {
  ...
  private static looksLikeSchool(line: string): boolean {
    const schoolKeywords = [
      ...
      '科技', '理工', '师范', '医科', '财经', '政法', '农林', '艺术',
    ];
    return schoolKeywords.some(kw => line.includes(kw)) && line.length > 2 && line.length < 50;
  }

  private static parseExperienceLine(line: string, currentExperience: any): void {
    if (!currentExperience.company && this.looksLikeCompany(line)) {
      currentExperience.company = line;
      return;
    }
    if (!currentExperience.position && /职位|岗位|Position|Job|Role/i.test(line)) {
      currentExperience.position = line.replace(/职位|岗位|Position|Job|Role|[:：]\.*/gi, '').trim();
      return;
    }
    if (!currentExperience.period && /\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{4}\.\d{1,2}\.\d{1,2}|至今|Present/i.test(line)) {
      currentExperience.period = line;
      return;
    }
    if (currentExperience.company && line.length > 10 && !this.looksLikeCompany(line)) {
  ...
  private static looksLikeCompany(line: string): boolean {
    ...
    const excludeKeywords = ['职位', '岗位', '时间', '描述', '责任', '职责', '完成'];
    if (excludeKeywords.some(kw => line.includes(kw))) {
      return false;
    }
    const companySuffixes = [
      '有限公司', '股份公司', '集团', '科技', '网络', '软件', '技术',
      'Co.', 'Ltd.', 'Inc.', 'Corp.', 'Group', 'Tech', 'Net',
    ];
    return companySuffixes.some(suffix => line.includes(suffix)) || /^[A-Z]/.test(line);
  }
  ...
  private static parseSkillsLine(line: string, skills: string[]): void {
    ...
    keywords.forEach(kw => {
      ...
      if (skill.length > 1 && skill.length < 30 && !skills.includes(skill)) {
        const excludeKeywords = ['的', '和', '与', '等', '擅长', '精通', '熟悉', '包括'];
        if (!excludeKeywords.some(ex => skill === ex || skill.startsWith(ex + ' '))) {
          skills.push(skill);
        }
      }
    });
